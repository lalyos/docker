...
docker run --name docker-hack -it --privileged -e TESTFLAGS -e TESTDIRS -e DOCKER_GRAPHDRIVER -e DOCKER_EXECDRIVER -e DOCKER_CROSSPLATFORMS="linux/amd64  darwin/amd64" --entrypoint /bin/bash docker-dev:latest -c 'git pull --rebase origin master && hack/make.sh binary cross'
